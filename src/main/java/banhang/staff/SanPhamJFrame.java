...
//                item.getMaloai(),
    ...
    public void addSPToSales() {
        ...
        SanPham sp = items.get(selectedRow);

        while (true) {
            String input = XDialog.prompt("Nhập số lượng cần bán:");

            if (input == null) {
                // Người dùng nhấn Cancel
                XDialog.alert("Bạn đã hủy thao tác!");
                return;
            } else if (input.trim().isEmpty()) {
                // Người dùng nhấn OK nhưng không nhập gì
                ...
                try {
                    ...
                    if (soluong <= 0) {
                        ...
                        chitiet.setSohd(this.hoadon.getSohd()); // Gán lại mã hóa đơn thực tế nếu có
                        chitiet.setMasp(sp.getMasp());
                        chitiet.setDongia(sp.getGia());
                        chitiet.setSoluong(soluong);

                        new banhang.dao.impl.CTHDDAOImpl().create(chitiet);
                        XDialog.alert("Đã thêm sản phẩm vào hóa đơn!");
                        this.dispose();
                        return;
                    }
    ...
    public void setHoaDon(HoaDon hoadon) {
        ...
        HoaDon hdDB = hoadonDao.findById(hoadon.getSohd());

        // Nếu chưa tồn tại thì tạo mới hóa đơn trong DB và lấy lại hóa đơn đã tạo (để có SOHD)
        if (hdDB == null) {
            HoaDon newHD = hoadonDao.create(hoadon); // tạo mới -> trả về hóa đơn đã có SOHD
            this.hoadon = newHD;
        } else {
            this.hoadon = hdDB;
        }
